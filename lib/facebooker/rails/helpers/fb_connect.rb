    ...
    module Helpers
      module FbConnect
        ...
        def init_fb_connect(*required_features,&proc)
          ...
          if request.ssl?
            init_string = "FB.init('#{Facebooker.api_key}', '#{(Pathname.new(root_url(:locale => nil)) + 'xd_receiver_ssl.html').to_s}', #{options[:app_settings]});"
          else
            init_string = "FB.init('#{Facebooker.api_key}', '#{(Pathname.new(root_url(:locale => nil)) + 'xd_receiver.html').to_s}', #{options[:app_settings]});"
          end
